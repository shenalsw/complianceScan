...
def install_package():
    ...
    if not check_package(package_name):
        print("libpam-pwquality package is currently NOT installed on this machine.")
        response = input("Would you like to install the package? *Recommended* Y/N ")
        if response.lower() == 'y':
            print("Installing libpam-pwquality Package now...")
            subprocess.run(['sudo', 'apt', 'install', package_name], check=True)
            print("Successfully Installed libpam-pwquality Package..")
        ...
        print(f"{package_name} is already installed. No changes were made.")
        report_file.write("\n1- The required package was already installed on machine. No changes were made.\n")
        report_file.flush()
...
def check_update_encrypt_method():
    ...
    if any(sha512_line in line for line in lines):
        ...
        report_file.write("\n7- The Default password encryption alogirthm meets standards. No changes were made.\n")
    else:
